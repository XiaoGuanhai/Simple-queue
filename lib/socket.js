    ...
    start: function(){
        ...
        io.on('connection',function(socket){
            ...
            if (typeof(socket.handshake.headers['token']) !== 'undefined'
                && socket.handshake.headers['token'] === process.env.APP_SECRET) {
                socket.join(process.env.APP_SECRET, function(){
                    logger.trace(`set "${socket.id}" join room (${process.env.APP_SECRET}) ok.`);
                });
            }
            socket.on('job progress', function(data){
                ...
                redisClient.get(data.id, function(err, reply){
                    ...
                    // 发送到内部房间
                    socket.to(process.env.APP_SECRET).emit('progress', data);
                });
            });
