...
Socket.prototype.start = function(callback){
    ...
    io.on('connection',function(socket){
        ...
        let unknown = true;
        let token = `client_` + (new Date().getTime());
        if (typeof handshake.query.token !== 'undefined'
        || handshake.query.token !== process.env.APP_SECRET) {
            unknown = false;
        ...
        if (unknown === true) {
            socket.join('unknown', function(){
                logger.trace(`set [token=${token} id=${socket.id}] join room (unknown) ok.`);
            });
        }
        // 回调
...
Socket.prototype.reply = function(event, data, socket) {
    ...
    // 找到了,发送进度给对应在线用户
    if (typeof self.clients[data.subscriber] !== 'undefined') {
        const reply = self.clients[data.subscriber];
        logger.trace(`found ["${data.subscriber}":"${reply}"]`);
