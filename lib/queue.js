...
module.exports = {
    ...
    start: function(){
        ...
        /**日志文件路径 */
        const logger = self.config.logger('queue');
        /** 将队列连接到Socket服务 */
        const socket = require('socket.io-client')(self.config.config.socket.server + ':' + self.config.config.socket.port, {
            transportOptions: {
                polling: {
                    extraHeaders: {
        ...
        socket.on('progress', function(data){
            ...
            if (typeof data.job.id === 'undefined') {
                logger.error(`undefined [job.id] in data for progress socket.`);
                return false;
            }
        ...
        queue.process('service', 10, function(job, done){
            ...
            request({
                ...
                if(error){
                    logger.warn(`request url: ${job.data.url} msg: ${error.toString()}`);
                    done(error.toString());
                    return false;
                }
            ...
            kue.Job.get(id, function(err, job) {
                ...
                if (typeof job.data.complete !== 'undefined' && job.data.complete.length > 0) {
                    ...
                    request({
                        ...
                        if(error){
                            logger.warn(`job complete request url: ${job.data.url} msg: ${error.toString()}`);
                            done(error.toString());
                            return false;
                        }
                        if(response.statusCode !== 200){
            ...
            kue.Job.get(id, function(err, job){
                ...
                if (typeof job.data.failed !== 'undefined' && job.data.failed.length > 0) {
                    ...
                    request({
                        ...
                        if(error){
                            logger.warn(`job failed request url: ${job.data.url} msg: ${error.toString()}`);
                            done(error.toString());
                            return false;
                        }
                        if(response.statusCode !== 200){
        ...
        // 监控端口
        server.listen(self.config.config.queue.port);
        //
        self.queue = queue;
        self.socket = socket;
    },
