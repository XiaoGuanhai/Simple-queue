...
function Queue(config) {
     ...
     * @type {Socket.io}
     */
    this.socket = require('socket.io-client')(this.config.socket.server + ':' + this.config.socket.port, {
        transportOptions: {
            polling: {
                extraHeaders: {
 ...
 * @param jobErr
 */
Queue.prototype.doFailed = function(id, jobErr) {
    let self = this;
 ...
 * @param {$ObjMap} res
 */
Queue.prototype.doComplete = function(id, res) {
    let self = this;
    kue.Job.get(id, function(err, job) {
        if (err) return;
        /** 通知Socket服务器 */
 ...
 * @returns {boolean}
 */
Queue.prototype.doProcess = function(serviceName, job, done) {
    /**
     *
     * @type {Queue}
     */
    ...
    if (self.isConnected) {
        // 添加Socket客户端ID
        job.data.job.socket = self.socket.id;
    }
    // 执行回调
    request({
        url: job.data.url,
...
Queue.prototype.start = function(doServices){
    ...
    for (let serviceName in doServices){
        ...
        queue.process(serviceName, serviceMaximum, function(job, done) {
            self.doProcess(serviceName, job, done);
        });
    }
    // 队列完成
    queue.on('job complete', function(id, res){
        self.doComplete(id, res);
    });
    // 队列重试失败
 ...
 * Socket客户端
 */
Queue.prototype.socketSubscribed = function(){
    const self = this;
    /** 连接Socket服务成功 */
